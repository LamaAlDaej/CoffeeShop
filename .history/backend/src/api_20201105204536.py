...
def add_drink(payload):
    ...
    Before looping the ingredients, we must make sure that the recipe is a list (if it has one ingredient it will be a string and this error will occur: TypeError: string indices must be integers)
    """
...
def update_drink(payload, id):
    ...
    if 'recipe' in body:
        ...
        # Check if each ingredient in the recipe contains the drink.long() data representation
        # According to (drink-form.component.html) Ingredient Name, Number of Parts, and Color must be filled
        # Before looping the ingredients, we must make sure that the recipe is a list
        # (if it has one ingredient it will be a string and this error will occur: TypeError: string indices must be integers)
        if isinstance(recipe, list):
            for ingredient in recipe:
                if (ingredient['name'] is None) or (ingredient['parts'] is None) or (ingredient['color'] is None):
                    # If fields are missing, send an error (unprocessable - 422)
